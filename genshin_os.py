...
def process():
    ...
    if os.environ.get('OS_COOKIE', '') != '':
        ...
        log.error(
            "Cookie not set properly, please read the documentation on how to set and format your cookie in Github Secrets.")
    ...
    notify.send(
        status=f'\nThời gian thực hiện: {dateTime_VN.strftime("%d/%m/%y %H:%M:%S")} \n  -Số tài khoản đăng nhập thành công: {success_num} \n  -Số tài khoản đăng nhập không thành công: {fail_num}', msg=msg_list)
    if ret != 0:
