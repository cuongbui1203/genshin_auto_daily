...
class Sign(Base):
    ...
    def run(self):
        ...
        if info_list:
            ...
            awards = Roles(self._cookie).get_awards().get(
                'data', {}).get('awards')
            uid = self._uid
            # str(self._uid).replace(
            #     str(self._uid)[1:7], ' ▓ ▓ ▓ ▓ ▓ ▓ ▓ ', 1)
            ...
            if info_list.get('data', {}).get('is_sign') is True:
                ...
                message['status'] = f"Nhà Lữ Hành, bạn đã nhận phần thưởng của hôm nay rồi!!"
            ...
            if info_list.get('data', {}).get('first_bind') is True:
                ...
                #message['status'] = f'Đã xong! Vui lòng kiểm tra lại qua hòm thư trong Game'
                message_list.append(self.message.format(**message))
                return ''.join(message_list)
